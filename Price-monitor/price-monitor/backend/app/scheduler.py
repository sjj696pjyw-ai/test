...
def _claim_daily_run(job_name):
    ...
    try:
        ...
        logger.warning(f"[SCHEDULER] не удалось взять замок ({e}); пропуск")
        return False
...
def _nightly_price_update(app):
    ...
    with app.app_context():
        if not _claim_daily_run('nightly_prices'):
            logger.info("[SCHEDULER] Ночное обновление уже запущено другим воркером — пропуск")
            return
        try:
            from app.services.price_update_service import PriceUpdateService
            PriceUpdateService.update_all_analyses_prices()
        except Exception as e:
            logger.error(f"[SCHEDULER] Ночное обновление: критическая ошибка — {e}")
...
def start_scheduler(app):
    ...
    try:
        ...
        logger.warning(f"[SCHEDULER] APScheduler недоступен ({e}); планировщик не запущен")
        return None
    ...
    scheduler.start()
    logger.info("[SCHEDULER] Запущен. Ночное обновление цен: каждый день в 19:30 UTC.")
    return scheduler
