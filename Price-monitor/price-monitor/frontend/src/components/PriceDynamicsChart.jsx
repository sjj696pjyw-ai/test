...
export function PriceDynamicsChart({ data, selectedUserProductId, onFilterChange, title }) {
  ...
  }, [availableUserProducts, filterProduct, handleFilterChange])
    ...
    filteredData.forEach((series, index) => {
      ...
      ;[
        ...
        if (
          firstIn &&
          prevOut &&
          firstIn.date > effStart &&
          byDate.get(effStart)[key] === undefined
        ) {
  ...
  const xTickInterval = chartData.length <= 14 ? 0 : Math.ceil(chartData.length / 12) - 1
        ...
        {productLegends.map((legend, idx) => (
          <div
            ...
            className={`flex items-center gap-2 transition-opacity cursor-pointer ${
              highlightedLegend !== null && highlightedLegend !== legend.dataKey
                ? 'opacity-40'
                : 'opacity-100'
            }`}
          ...
          >
            <div
              className={`w-3 h-3 rounded-full ${
                highlightedLegend === legend.dataKey
              ...
              style={{
                ...
                boxShadow:
                  highlightedLegend === legend.dataKey ? `0 0 8px ${legend.color}` : 'none',
  ...
  const CustomDot = (props) => {
    ...
    const isHovered = activeDot && activeDot.dataKey === dataKey && activeDot.index === index
  ...
  const orderedLines = highlightedLegend
    ? [
        ...lines.filter((l) => l.key !== highlightedLegend),
        ...lines.filter((l) => l.key === highlightedLegend),
      ]
    : lines
